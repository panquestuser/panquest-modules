...
static int tg3_ports = 2;
module_param(tg3_ports, int, 0);
MODULE_PARM_DESC(tg3_ports, "Dual-port Mode = 2, Single-port Mode - port 1 = 0, port 2 = 1");
#endif
...
#ifdef CONFIG_MACH_QNAPTS
	if (tg3_ports == 0 || tg3_ports == 1) {
		if ((pdev->devfn & 0x01) != tg3_ports) {
			return -ENODEV;
		}
	}
#endif
